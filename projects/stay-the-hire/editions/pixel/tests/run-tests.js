...
ok(STH.ROLES.length === 8, `8 roles (found ${STH.ROLES.length})`);
STH.ROLES.forEach(r => r.deck.forEach(id => ok(STH.CARD[id], `role ${r.id}: deck card ${id} exists`)));

/* art coverage: every card has an icon, every enemy & role has a portrait */
STH.CARDS.forEach(c => ok(STH.CARD_ICONS[c.id], `card ${c.id}: has an icon`));
