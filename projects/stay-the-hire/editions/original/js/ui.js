    ...
    return `<div class="combat">
      ...
      <div class="battlerow">
        <div class="playerpane">
          <div class="pportrait">${STH.roleArt(run.role)}<span class="pname">${esc(run.roleName)}</span></div>
          <div class="pstat" data-tip="${esc(STH.RESOURCE_HELP.influence)}">⚡ <b>${c.influence}</b> Influence</div>
          <div class="pstat" data-tip="${esc(STH.RESOURCE_HELP.trust)}">🤝 <b>${c.trust}</b> Trust</div>
  ...
  function runFx(run) {
    ...
    q.forEach((ev, k) => {
      setTimeout(() => {
        ...
        else if (ev.t === 'phit') { host = $('.playerpane'); cls = 'dmg'; txt = '+' + ev.n + ' 🔥'; }
        else if (ev.t === 'block' || ev.t === 'pblock') { host = $('.playerpane'); cls = 'blk'; txt = '🛡️ ' + ev.n; }
        else if (ev.t === 'heal') { host = $('.playerpane'); cls = 'heal'; txt = '−' + ev.n + ' 🔥'; }
        if (!host) return;
        if (cls === 'dmg') {
