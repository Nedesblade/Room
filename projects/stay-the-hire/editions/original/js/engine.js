...
(function (g) {
  ...
  const POWER_CARDS = new Set(['delegate', 'task_force', 'take_notes', 'deep_work_sprint']);
  ...
  function startShop(run) {
    const cards = rollCardChoices(run, 4, false).map(id => ({ id, price: shopPrice(STH.CARD[id].rarity), sold: false }));
  ...
  STH.buyShopPerk = function (run) {
    ...
    if (!it || it.sold || run.pc < it.price) return false;
    run.pc -= it.price; it.sold = true;
    run.perks.push(it.id);
    if (it.id === 'standing_desk') run.maxStress += 10;
    if (it.id === 'mentor_network') run.luck += 2;
    return true;
  };
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (isBurnoutCard(card.id)) {
        ...
        if (perkHas(run, 'fire_drill') && !c.flags.firstBurnoutExhausted) {
          c.flags.firstBurnoutExhausted = true; c.exhaust.push(card);
      ...
      if (def.onDrawStress) {
        ...
        if (def.exhaustOnDraw) { c.exhaust.push(card); continue; }
      }
      c.hand.push(card);
      if (run.over) return;
    }
  ...
  STH.playCard = function (run, uid, targetIdx) {
    ...
    (v.exhaust ? c.exhaust : c.discard).push(inst);
    checkCombatEnd(run);
    return true;
  };
      ...
      if (fx.removeBurnout) {
        ...
        for (const pile of [c.hand, c.discard, c.draw]) {
          for (let j = pile.length - 1; j >= 0 && left > 0; j--) {
            if (isBurnoutCard(pile[j].id)) { c.exhaust.push(pile.splice(j, 1)[0]); left--; }
          }
        }
  ...
  STH.endTurn = function (run) {
    ...
    for (const inst of c.hand) {
      ...
      if (def.endTurnInHand) {
        ...
        if (run.over) return;
        if (e.exhaust) { c.exhaust.push(inst); continue; }
      }
  ...
  STH.takePerkReward = function (run, perkId) {
    ...
    if (perkId) {
      ...
      if (perkId === 'standing_desk') run.maxStress += 10;
      if (perkId === 'mentor_network') run.luck += 2;
    }
  ...
  function applyEventFx(run, fx) {
    ...
    if (fx.perkRandom) {
      ...
      if (avail.length) {
        ...
        run.perks.push(p.id);
        if (p.id === 'standing_desk') run.maxStress += 10;
        if (p.id === 'mentor_network') run.luck += 2;
