...
ok(text().includes('THE INFINITE ORG CHART'), 'world name on the menu');
...
ok(doc.querySelectorAll('[data-role]').length === 9, 'all 9 roles offered');
click(doc.querySelector('[data-role="developer"]'));

/* map */
ok(text().includes('Infinite Org Chart'), 'org chart map renders');
ok(text().includes('The Onboarding Wastes'), 'Act 1: The Onboarding Wastes shown');
...
if (text().includes('Encounter Survived')) {
  ...
  ok(text().includes('Infinite Org Chart'), 'card reward claimed, back on the map');
}
