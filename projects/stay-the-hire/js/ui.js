...
(function (g) {
  ...
  function resourceBar(run) {
    ...
    return `<div class="resbar">
      ...
      <span class="perkrow bar">${run.perks.map(id => { const p = STH.PERK[id]; return `<span class="chip perk" data-tip="<b>${p.icon} ${esc(p.name)}</b><br>${esc(p.desc)}">${p.icon}</span>`; }).join('')}</span>
      <div class="res act">Act ${run.act}/3 · ${['', 'The Onboarding Wastes', 'The Middle Management Mire', 'The Executive Cloud Castle'][run.act]}</div>
    </div>`;
  }
  ...
  function cardHTML(run, inst, opts = {}) {
    ...
    const icon = STH.cardPixelIcon(v.id);
  ...
  function nameScreen() {
    return `<div class="menu">
      ...
      <div class="panel name-panel">
        <div class="npcport">${STH.npcSprite('handbook')}</div>
        <h2>📖 The Haunted Handbook</h2>
        <p>An employee handbook flutters open by itself, sticky-note eyes blinking. “Welcome, valued contributor. Please ignore the screaming in the calendar. Previous hires usually dissolved by now.”</p>
        <p>It requires exactly one field. The rest of the form was lost in a migration.</p>
    ...
    return `<div class="menu wide">
      ...
      <div class="roles">
        ${STH.ROLES.map(r => `<div class="role" data-role="${r.id}">
          <div class="rportrait">${STH.roleSprite(r.id)}</div>
  ...
  function mapScreen(run) {
    ...
    return `<div class="mapwrap">
      <h2 class="acttitle">▦ The Infinite Org Chart — Act ${run.act}: ${['', 'The Onboarding Wastes', 'The Middle Management Mire', 'The Executive Cloud Castle'][run.act]}</h2>
  ...
  function enemyHTML(run, e, idx) {
    ...
    return `<div class="enemy ${e.hp <= 0 ? 'dead' : ''} ${targetable ? 'targetable' : ''}" data-enemy="${idx}">
      ...
      <div class="eportrait">${STH.enemySprite(e.id)}</div>
    ...
    if (c.boss && !c._introShown) {
      ...
      return `<div class="bossintro">
        <div class="panel bosspanel">
          <div class="bossport">${STH.enemySprite(boss.id)}</div>
          <h2>${esc(boss.name)}</h2>
    ...
    return `<div class="combat">
      <div class="arena">
        <div class="playerpane heroside">
          <div class="heroholder">${STH.roleSprite(run.role, hp <= 0 ? 'ko' : 'idle')}</div>
  ...
  function eventScreen(run) {
    ...
    if (run.event.resolved) {
      return `<div class="panel event">
        ...
        ${run.seenEvents.length % 4 === 1 ? `<p class="fineprint alfredline">${STH.npcSprite('alfred')} Alfred wanders past, badge reading “${['Finance','Facilities','Legal','Vibes','Unknown'][run.seenEvents.length % 5]}” this week: “${esc(['I was on this project before it had a name.','Ask Finance. Unless I’m Finance this week.','That dashboard used to be a person.','I’m not in the org chart. The org chart is in me.'][run.seenEvents.length % 4])}”</p>` : ''}
  ...
  function restScreen(run) {
    ...
    return `<div class="panel">
      <div class="npcport duo">${STH.npcSprite('printer_elder')}${STH.npcSprite('goblin')}</div>
      <h2>☕ The Break Room</h2>
      <p>An ancient printer with a paper-jam beard regards you. The Printer Elder rumbles: “I have printed things your managers have forgotten.” From behind the microwave, the Break Room Goblin adds: “Someone microwaved fish in 2019. The building remembers.”</p>
      <div class="choices">
  ...
  function promotionScreen(run) {
    return `<div class="panel">
      ...
      <p><b>Act ${run.act}: ${['', 'The Onboarding Wastes', 'The Middle Management Mire', 'The Executive Cloud Castle'][run.act]}</b></p>
  ...
  function summaryScreen(run) {
    ...
    return `<div class="panel summary">
      ...
      <div class="heroholder sumhero">${STH.roleSprite(run.role, s.won ? 'cheer' : 'ko')}</div>
  ...
  function shopScreen(run) {
    ...
    return `<div class="panel wide shop">
      <div class="npcport">${STH.npcSprite('oracle')}</div>
      <h2>🧙 The Office Supply Closet</h2>
      <p>A tiny robed figure surrounded by staplers, mugs and forbidden toner. The Office Supply Oracle speaks: “Budget is a social construct until Procurement arrives.” You have ${run.pc} Political Capital and the moral flexibility to spend it.</p>
      <h3>Cards</h3>
      <div class="cardopts">
  ...
  function mysteryScreen(run) {
    return `<div class="panel">
      <div class="npcport">${STH.npcSprite('mimic')}</div>
      <h2>❔ Unmarked Calendar Hold</h2>
      <p>A cardboard box with teeth and a company hoodie waits inside. The Swag Box Mimic burps politely.</p>
