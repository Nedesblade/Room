...
(function (g) {
  ...
  /* =============== ROLE STARTERS =============== */
  card({ id:'eager_volunteer', name:'Eager Volunteer', cost:0, cat:'Social', rarity:'starter', target:'enemy', innate:true,
    fx:[{p:3},{stress:1}], text:'Innate. Deal 3 Pressure. Gain 1 Stress.', flavor:'Your hand was up before the question ended.',
    up:{ fx:[{p:6},{stress:1}], text:'Innate. Deal 6 Pressure. Gain 1 Stress.' } });
  ...
  card({ id:'circle_back', name:'Circle Back', cost:1, cat:'Politics', rarity:'common', target:'none',
    fx:[{draw:2},{discardRandom:1}], text:'Draw 2 cards, then discard 1 at random.', flavor:'A threat disguised as time management.',
    up:{ fx:[{draw:3},{discardRandom:1}], text:'Draw 3 cards, then discard 1 at random.' } });
  ...
  card({ id:'leftover_cake', name:'Leftover Kitchen Cake', cost:0, cat:'Recovery', rarity:'common', target:'none', exhaust:true, ethereal:true,
    fx:[{heal:3}], text:'Ethereal. Heal 3 Stress. Exhaust.', flavor:'Someone\u2019s farewell cake. They left an hour ago. It\u2019s fair game.',
    up:{ fx:[{heal:5},{t:2}], text:'Ethereal. Heal 5 Stress. Gain 2 Trust. Exhaust.' } });

  card({ id:'per_my_last_email', name:'Per My Last Email', cost:1, cat:'Politics', rarity:'common', target:'enemy',
    fx:[{p:7},{eStatus:{id:'onRecord',n:1}}], text:'Deal 7 Pressure. Apply 1 On the Record.', flavor:'A dagger, but with timestamps.',
    up:{ fx:[{p:10},{eStatus:{id:'onRecord',n:1}}], text:'Deal 10 Pressure. Apply 1 On the Record.' } });
  card({ id:'quick_sync', name:'Quick Sync', cost:0, cat:'Productivity', rarity:'common', target:'none', exhaust:true,
    fx:[{t:3},{draw:1}], text:'Gain 3 Trust. Draw 1 card. Exhaust.', flavor:'“Quick” is doing 40 minutes of work in that sentence.',
    up:{ fx:[{t:5},{draw:1}], text:'Gain 5 Trust. Draw 1 card. Exhaust.' } });
  card({ id:'deck_alignment', name:'Deck Alignment', cost:2, cat:'Executive', rarity:'uncommon', target:'none',
    fx:[{status:{id:'aligned',n:1}}], text:'Gain 1 Aligned: at the start of each turn, if you played 3+ cards last turn, gain 1 In the Loop.', flavor:'The slides now agree with each other. The people remain at war.',
    up:{ cost:1, fx:[{status:{id:'aligned',n:1}}], text:'Gain 1 Aligned: at the start of each turn, if you played 3+ cards last turn, gain 1 In the Loop.' } });
  card({ id:'calendar_conflict', name:'Hide Behind a Calendar Conflict', cost:1, cat:'Recovery', rarity:'common', target:'none',
    fx:[{t:8}], text:'Gain 8 Trust.', flavor:'You cannot be attacked if you are, technically, in another meeting.',
    up:{ fx:[{t:11}], text:'Gain 11 Trust.' } });
  ...
  /* Index by id */
  card({ id:'meeting_invite', name:'Meeting Invite', cost:0, cat:'Status', rarity:'status', target:'none', unplayable:true,
    fx:[], text:'Unplayable. It just sits there. Recurring. 4:30pm.', flavor:'“Quick sync” — agenda: none, attendees: 19, declining: political.' });
  card({ id:'corporate_debt', name:'Corporate Debt', cost:0, cat:'Status', rarity:'status', target:'none', unplayable:true,
    fx:[], text:'Unplayable. When combat ends, lose 2 Reputation unless this was exhausted.', flavor:'Signed by someone who left in 2021. Countersigned by the void.' });

  const byId = {};
  C.forEach(c => { byId[c.id] = c; });
  g.STH.CARDS = C;
  g.STH.CARD = byId;
  g.STH.BURNOUT_IDS = ['burnout','doomscroll','cynicism','bad_sleep','eod_request','pip_paperwork','cringe_card','meeting_invite','corporate_debt'];
